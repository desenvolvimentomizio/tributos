...
export default defineComponent({
  ...
  setup() {
    ...
    const mapCSTCSOSN = {
      ...
      '201': 'Simples c/ crédito + ST',
      '202': 'Simples s/ crédito + ST',
      '203': 'Isenção no Simples + ST',
      '300': 'Imune',
    ...
    const mapPISCOFINS = {
      ...
      '56': 'Crédito vinculado a receitas tributadas e não-tributadas no mercado interno e de exportação',
    ...
    // ------- Rules helpers (precisam retornar true) -------
    const ruleBuscarDescricaoCSTICMCSOSN = (val) => {
      if (val) {
        // tenta ICMS e, se não achar, tenta CSOSN
        descricaocst_icm_csosn.value = mapCSTICM[val] || mapCSTCSOSN[val] || 'NÃO ACHOU'
      } else {
