  ...
  <q-page padding>
    <q-form class="row justify-center" @submit.prevent="handlePasswordReset">
      ...
      <div class="col-md-4 col-sm-6 col-xs-10 q-gutter-y-md">
        <q-input label="Nova Senha" v-model="password" :type="isPwd ? 'password' : 'text'" lazy-rules
        ...
        <div class="full-width q-pt-md q-gutter-y-sm">
          <q-btn label="Enviar Nova Senha" color="primary" class="full-width" outline rounded type="submit" />
        </div>
      </div>
    </q-form>
  </q-page>
    ...
    const handlePasswordReset = async () => {
      try {
        ...
        if (error) throw error
                notifySuccess('Senha atualizada com sucesso')
        router.push({ name: 'login' }) // ajuste para sua rota
      } catch (error) {
                notifyError(error?.message || 'Falha ao atualizar a senha')
      }
    }
